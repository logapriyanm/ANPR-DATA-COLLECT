...
MODEL_PATH = Path(r"f:\ANPR\ANPR-NEW\ES-ANPR-CMDS-AI-BACKEND-SERVICE\models\vehicle_type.pt")
INPUT_DIR = Path(r"f:\ANPR\ANPR-NEW\ES-ANPR-CMDS-RTSP-BACKEND-SERVICE\captures")
OUTPUT_DIR = BASE_DIR / "Twowheeler3"
...
# ── CLASSES ───────────────────────────────────────────────────────────────────
TWO_WHEELER_KEYWORDS = {'motorcycle', 'scooter', 'bike', 'two_wheeler', 'bicycle'}
...
def main():
    print("=" * 80)
    print(" ANPR DATA COLLECT - TWO-WHEELER CROPPING MODEL & PIPELINE")
    print("=" * 80)
    ...
    for ext in image_extensions:
        image_paths.extend(glob.glob(str(INPUT_DIR / "**" / ext), recursive=True))
    ...
    for i in range(0, total_images, BATCH_SIZE):
        ...
        for img_idx, res in enumerate(results):
            ...
            for box_idx, box in enumerate(boxes):
                ...
                if crop.size > 0:
                    tw_count_in_img += 1
                    crop_filename = f"{base_name}_tw{tw_count_in_img:02d}_{cname}_conf{int(conf*100)}.jpg"
